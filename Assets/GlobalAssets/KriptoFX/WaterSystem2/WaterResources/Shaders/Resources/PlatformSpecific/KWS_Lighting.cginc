...
#define KWS_LIGHTING_COMMON_INCLUDED
...
int KWS_DirLightShadowStableFit;
int KWS_DirLightShadowCascades;
int KWS_UseDirLightShadow;
Texture2D KWS_DirLightShadowMap0;
float4 KWS_DirLightShadowMap0_TexelSize;

TextureCube KWS_PointLightShadowMap0;
TextureCube KWS_PointLightShadowMap1;
TextureCube KWS_PointLightShadowMap2;
TextureCube KWS_PointLightShadowMap3;

Texture2D KWS_SpotLightShadowMap0;
Texture2D KWS_SpotLightShadowMap1;
Texture2D KWS_SpotLightShadowMap2;
Texture2D KWS_SpotLightShadowMap3;

struct KWS_LightData
{
	float4 color;
	float range;
...
StructuredBuffer<KWS_LightData> KWS_PointLightsBuffer;
StructuredBuffer<KWS_LightData> KWS_SpotLightsBuffer;
...
uint KWS_ShadowSpotLightsCount;
    ...
    #define OVERRIDE_FORCECASE   [forcecase]
#else
    #define OVERRIDE_FORCECASE
#endif
...
{
	OVERRIDE_FORCECASE switch(shadowIdx)
	{
		case 0: return KWS_PointLightShadowMap0.SampleLevel(shadowSampler_LinearClamp, coord, 0) < dist ? 0 : 1;
		case 1: return KWS_PointLightShadowMap1.SampleLevel(shadowSampler_LinearClamp, coord, 0) < dist ? 0 : 1;
		case 2: return KWS_PointLightShadowMap2.SampleLevel(shadowSampler_LinearClamp, coord, 0) < dist ? 0 : 1;
		case 3: return KWS_PointLightShadowMap3.SampleLevel(shadowSampler_LinearClamp, coord, 0) < dist ? 0 : 1;
		default: return 0;
	}
}
...
inline float3 getShadowCoord(uint lightIndex, float4 wpos, half4 cascadeWeights)
{
	...
	#endif
	return shadowMapCoordinate.xyz;
}
...
inline half KWS_AngleAttenuation(half3 spotDirection, half3 lightDirection, half2 spotAttenuation)
{
	// urp version
...
{
	KWS_LightData light = KWS_PointLightsBuffer[lightIndex];
...
{
	KWS_LightData light = KWS_SpotLightsBuffer[lightIndex];
	...
	half attenuation = DistanceAttenuation(distanceSqr, light.range) * KWS_AngleAttenuation(-light.forward.xyz, lightDirection, light.attenuation.zw);
	return attenuation;
}
