...
namespace KWS
{
    internal static class ReflectionUtils
    {
        ...
        public static GameObject CreateReflectionCamera(string name,  out Camera reflectionCamera, out Transform reflectionCamTransform)
        {
        ...
        public static GameObject CreateDepthCamera(string name, out Camera depthCamera)
        {
            var currentCamGO = KW_Extensions.CreateHiddenGameObject(name);
            
            depthCamera                  = currentCamGO.AddComponent<Camera>();
            depthCamera.cameraType       = CameraType.Reflection;
        ...
        public static void RenderPlanarReflection(Camera currentCamera, ScriptableRenderContext context, Vector3 waterWorldPos, Camera reflectionCamera, Transform reflectionCameraTransform, RenderTexture target, bool useStereo, Camera.StereoscopicEye eye = Camera.StereoscopicEye.Left)
        {
            var     normal          = Vector3.up;
            float   d               = -Vector3.Dot(normal, waterWorldPos) - m_ClipPlaneOffset;
            Vector4 reflectionPlane = new Vector4(normal.x, normal.y, normal.z, d);

            Matrix4x4 reflection = Matrix4x4.zero;
            CalculateReflectionMatrix(ref reflection, reflectionPlane);
            var reflectedCameraPos = currentCamera.transform.position - new Vector3(0, waterWorldPos.y * 2, 0);
            reflectedCameraPos.y = -reflectedCameraPos.y;

            SetCameraClampedPosRotation(reflectionCameraTransform, reflectedCameraPos, currentCamera.transform.rotation);

            if (useStereo)
            {
                reflectionCamera.worldToCameraMatrix = currentCamera.GetStereoViewMatrix(eye) * reflection;
                var clipPlane = CameraSpacePlane(reflectionCamera, waterWorldPos, normal, 1.0f);
            ...
            try
            {
               ...
               KWS_CoreUtils.UniversalCameraRendering(reflectionCamera, context);
            }
            finally
            {
        ...
        class PlanarReflectionSettingData
        {
            ...
            public void Set()
            {
                ...
                if (WaterSystem.QualitySettings.RenderPlanarShadows == false) QualitySettings.shadows = ShadowQuality.Disable;
            }
