...
//
// Vertex Stage: update fur mover data in a buffer
//
Attributes vert(_Attributes input)
{
    ...
    if (abs(time - data.time) > 1e-3)
    {
        ...
        data.dPosWS = (move < 0.01 ? data.dPosWS : (min(move, 1.0) / max(move, 0.01)) * data.dPosWS);
        data.time = time;
...
//
// Helper function to output a vertex into the stream.
//
void AppendVertex(
...
//
// Geometry Shader: build the fur geometry by interpolating between the base and the moved fur tip
//
[maxvertexcount(45)]
void geom(triangle Attributes input[3], inout TriangleStream<Varyings> stream)
{
    // --- Precompute per-triangle values ---
    uint id0 = input[0].id;
    uint id1 = input[1].id;
    uint id2 = input[2].id;
    ...
    float2 topLightmapUv = (prevLightmapUv0 + prevLightmapUv1 + prevLightmapUv2) / 3.0;
    ...
    // Initialize working positions, normals, and centers.
    ...
    // Add some randomness to the face normal.
    ...
    float3 vertMoveWS = (_Buffer[id0].dPosWS + _Buffer[id1].dPosWS + _Buffer[id2].dPosWS) / 3.0;
    ...
    const float delta = 1.0 / _FurJoint; // assuming _FurJoint is a uniform

    // --- Loop over fur joints using integer counter ---
    for (int j = 0; j < _FurJoint; ++j)
    {
        float nextFactor = prevFactor + delta;
        float moveFactor = pow(abs(nextFactor), _BaseMove.w);
        // Interpolate between the original top position and the moved top position.
        float3 lerpMovedTopPosOS = lerp(topPosOS, topMovedPosOS, moveFactor);

        // Compute offset vectors for each vertex.
        ...
        float3 nextCenterPosOS = (nextPos0OS + nextPos1OS + nextPos2OS) / 3.0;

        float3 basePosOS = (nextCenterPosOS + prevCenterPosOS) / 2.0;
        // Compute new normals as a blend between the original face normal and the direction
        // from the base position to the new vertex position.
        ...
        // Interpolate UVs and lightmap UVs.
        float2 nextUv0 = prevUv0 + uvInterp0 * delta;
        float2 nextUv1 = prevUv1 + uvInterp1 * delta;
        float2 nextUv2 = prevUv2 + uvInterp2 * delta;
        float2 nextLightmapUv0 = prevLightmapUv0 + lightmapUvInterp0 * delta;
        float2 nextLightmapUv1 = prevLightmapUv1 + lightmapUvInterp1 * delta;
        float2 nextLightmapUv2 = prevLightmapUv2 + lightmapUvInterp2 * delta;

        // Emit vertices in a pattern (you can adjust the order as needed).
        ...
        prevCenterPosOS = (nextPos0OS + nextPos1OS + nextPos2OS) / 3.0;
...
//
// Fragment Shader: blend textures and compute final lit color.
//
float4 frag(Varyings input) : SV_Target
{
    ...
    // Use a non‐linear blend for occlusion if desired.
