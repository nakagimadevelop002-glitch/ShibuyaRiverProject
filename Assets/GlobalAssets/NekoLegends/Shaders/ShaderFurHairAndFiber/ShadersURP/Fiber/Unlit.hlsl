...
{
    // --- Precompute base positions and UV data ---
    ...
    // Compute wind–affected (moved) top position.
    ...
    // --- Set up fur joint interpolation ---
    float prevFactor = 0.0;
    float delta = 1.0 / (float)_FurJoint; // _FurJoint should be an integer uniform

    // Iterate over each fur joint.
    for (int j = 0; j < _FurJoint; ++j)
    {
        float nextFactor = prevFactor + delta;
        // Compute a move factor for the current joint.
        float moveFactor = pow(abs(nextFactor), _BaseMove.w);
        // Interpolate the top position for this joint.
        float3 lerpTopPosOS = lerp(topPosOS, topMovedPosOS, moveFactor);

        // Compute new positions by offsetting each base vertex.
        ...
        // Emit vertices in a pattern that preserves triangle connectivity.
    ...
    // Emit final vertices to complete the fur strand.
    AppendVertex(stream, prevPos0OS, prevUv0, prevFactor);
    AppendVertex(stream, prevPos1OS, prevUv1, prevFactor);
...
{
    // Sample base texture and modulate with base color.
    float4 color = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.uv);
    color *= _BaseColor;
    // Apply occlusion blending based on the fur factor.
