...
// Structure holding tessellation factors and additional interpolated data.
...
HsConstantOutput hullConst(InputPatch<Attributes, 3> patch)
{
    HsConstantOutput o = (HsConstantOutput)0;

    // Compute a tessellation factor based on camera distance.
    float3 cameraPosMV = float3(UNITY_MATRIX_MV[0][3], UNITY_MATRIX_MV[1][3], UNITY_MATRIX_MV[2][3]);
    float distance = length(cameraPosMV);
    ...
    // Cache base positions and normals.
    ...
    // Compute intermediate barycentric control points.
    ...
    // Compute the average of the barycentrics and the vertex positions.
    ...
    // Compute coefficients for normal interpolation.
    float3 edge0 = p1 - p0;
    float3 edge1 = p2 - p1;
    float3 edge2 = p0 - p2;
    ...
    float3 bary : SV_DomainLocation)
{
    ...
    // Decompose barycentrics.
    float fU = bary.x;
    float fV = bary.y;
    float fW = bary.z;
    
    // Precompute squared and tripled terms.
    float fUU = fU * fU;
    float fVV = fV * fV;
    float fWW = fW * fW;
    float fUU3 = fUU * 3.0;
    ...
    // Compute the position using a weighted sum of the vertices, control points, and center.
    o.positionOS = float4(
          patch[0].positionOS.xyz * fWW * fW +
          patch[1].positionOS.xyz * fUU * fU +
          patch[2].positionOS.xyz * fVV * fV +
    ...
    // Compute the normal by blending vertex normals and the extra normals from the hull constant.
    o.normalOS = normalize(
          patch[0].normalOS * fWW +
          patch[1].normalOS * fUU +
          patch[2].normalOS * fVV +
          hsConst.f3N110 * fW * fU +
          hsConst.f3N011 * fU * fV +
          hsConst.f3N101 * fW * fV);
    
    // Interpolate texture coordinates and lightmap UVs.
    o.texcoord = patch[0].texcoord * fW + patch[1].texcoord * fU + patch[2].texcoord * fV;
    o.lightmapUV = patch[0].lightmapUV * fW + patch[1].lightmapUV * fU + patch[2].lightmapUV * fV;
    
    // Propagate the ID (assumed constant across the patch).
    o.id = patch[0].id;
